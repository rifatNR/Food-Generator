    ...
    return (
        <div className="absolute right-0 top-0 w-[500px] h-screen">
            <div className="relative flex flex-col space-y-10 h-full justify-center items-end">
                <RecepieCard isShow={ctx?.card_anim_index ? ctx.card_anim_index >= 1 : false}/>
                <RecepieCard isShow={ctx?.card_anim_index ? ctx.card_anim_index >= 2 : false}/>
                <RecepieCard isShow={ctx?.card_anim_index ? ctx.card_anim_index >= 3 : false}/>
            </div>
        </div>
    );
}
